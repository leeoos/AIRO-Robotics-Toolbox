...
disp("Maximum full rank for augmented jacobian")
full_rank_J_A
...
for k = (1 : NUM_OF_TASKS)
    ...
    % Augmented jacobian up until step k
    ...
    if (k == 1)
        ...
        if (rank(J_A_k) < full_rank_J_A)
            ...
            mu = 0.001;
            J_DLS = transpose(J_k*P_A{k-1})*(J_k*P_A{k-1}*transpose(J_k*P_A{k-1}) + mu*eye(size(J_k*P_A{k-1},1)))^(-1);
            ...
            P_NULL = J_NULL* (transpose(J_NULL)*J_NULL)^(-1) * transpose(J_NULL);
...
function dls_res = compute_q_dls(mu, J, q_dot, r_dot)
    J_DLS = transpose(J) * (J*transpose(J) + mu*eye(size(J,1)))^(-1);
